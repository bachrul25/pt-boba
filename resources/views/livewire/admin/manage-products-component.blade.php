...
<div>
    ...
    <div class="d-flex justify-content-between align-items-center mb-3 flex-wrap gap-2">
        <h5 class="fw-bold mb-0">Produk Fashion</h5>
        <div class="d-flex gap-2">
            <input type="text" wire:model.live.debounce.300ms="search" class="form-control form-control-sm" placeholder="Cari produk...">
            @if(! $showForm)
                <button class="btn btn-boba btn-sm" wire:click="newProduct"><i class="bi bi-plus-lg me-1"></i> Tambah</button>
            @endif
        </div>
    </div>
            ...
            <div class="card-body">
                <form wire:submit="save" class="row g-3">
                    ...
                    <div class="col-md-3 d-flex align-items-end">
                        <div class="form-check"><input class="form-check-input" type="checkbox" wire:model="is_active" id="pa"><label class="form-check-label small" for="pa">Aktif</label></div>
                    </div>
                    ...
                    <div class="col-md-6"><label class="form-label small fw-semibold">Stok</label>
    ...
    <div class="card">
        <div class="table-responsive">
            <table class="table table-sm align-middle mb-0">
                <thead><tr><th>Nama</th><th>Brand</th><th>Seller</th><th>Harga</th><th>Stok</th><th>Status</th><th></th></tr></thead>
                <tbody>
                    @forelse($products as $p)
                        <tr>
                            <td class="fw-semibold">{{ $p->name }}<br><small class="text-secondary">{{ $p->category }}</small></td>
                            <td>{{ $p->brand->name ?? '-' }}</td>
                            <td>{{ $p->seller->store_name ?? '— Pusat —' }}</td>
                            <td>Rp {{ number_format($p->price, 0, ',', '.') }}</td>
                            <td>{{ $p->stock }}</td>
                            <td>@if($p->is_active)<span class="badge badge-soft-success">Aktif</span>@else<span class="badge badge-soft-danger">Off</span>@endif</td>
                            <td class="text-end">
                                <button wire:click="edit({{ $p->id }})" class="btn btn-sm btn-outline-secondary"><i class="bi bi-pencil"></i></button>
                                <button wire:click="delete({{ $p->id }})" wire:confirm="Hapus produk?" class="btn btn-sm btn-outline-danger"><i class="bi bi-trash"></i></button>
                            </td>
                        </tr>
                    @empty
                        <tr><td colspan="7" class="text-center text-secondary small py-3">Belum ada produk.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
