...
<div>
    ...
    <div class="d-flex justify-content-between align-items-center mb-3">
        <h5 class="fw-bold mb-0">Layanan Saya</h5>
    ...
    @if($showForm)
        <div class="card mb-3"><div class="card-body">
            <form wire:submit="save" class="row g-3">
                ...
                <div class="col-md-4"><label class="form-label small fw-semibold">Unit</label>
                ...
                <div class="col-12">
                    <div class="form-check"><input class="form-check-input" type="checkbox" wire:model="is_active" id="ssa"><label class="form-check-label small" for="ssa">Aktif</label></div>
                </div>
    ...
    <div class="card"><div class="table-responsive">
        <table class="table table-sm align-middle mb-0">
            <thead><tr><th>Nama</th><th>Brand</th><th>Kategori</th><th>Harga</th><th></th></tr></thead>
            <tbody>
                @forelse($services as $s)
                    <tr>
                        ...
                        <td>{{ $s->brand->name ?? '-' }}</td>
                        <td>{{ $s->category }}</td>
                        <td>Rp {{ number_format($s->price, 0, ',', '.') }} <small class="text-secondary">/{{ $s->unit }}</small></td>
                        <td class="text-end">
                ...
                @empty
                    <tr><td colspan="5" class="text-center text-secondary small py-3">Belum ada layanan.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div></div>
